  ...
  def sample_tasks(self, n_tasks):
    ...
    for _ in range(n_tasks):
      ...
      if 'body_inertia' in self.rand_params:
        body_inertia_multiplyers = np.array(1.5) ** np.random.uniform(-self.log_scale_limit, self.log_scale_limit,
      ...
      if 'dof_damping' in self.rand_params:
        dof_damping_multipliers = np.array(1.3) ** np.random.uniform(-self.log_scale_limit, self.log_scale_limit,
      ...
      if 'geom_friction' in self.rand_params:
        dof_damping_multipliers = np.array(1.5) ** np.random.uniform(-self.log_scale_limit, self.log_scale_limit,
